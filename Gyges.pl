...
mover((Numero,Fila,Columna), Tablero, Sol):-delete(Tablero, (Ficha,Fila,Columna), NuevoTablero), %borrar la posicion antigua de la ficha
											findall(Res,moverAux((Numero,Fila,Columna), Numero, NuevoTablero,[(Fila,Columna)], Res),List), %encontrar todas las soluciones y asociarlo a List
											list_to_set(List,Set), %Eliminar las casillas repetidas.
											member(Sol,Set).		%Listar cada una de los posibles movimientos.
...
moverAux(Ficha, 0, Tablero, Mov, NuevoTablero) :- 	ocupado(Tablero, Ficha, Numero), %Si al final del movimiento se tiene una casilla ocupada
													moverAux(Ficha,Numero,Tablero,Mov, NuevoTablero).%volver a mover la ficha la cantidad de espacios que representa la ficha en esa posicion.
moverAux(Ficha, 0, Tablero, Mov, NuevoTablero) :- !, append(Tablero, [Ficha], NuevoTablero).%Agregar la nueva posicion de la ficha al tablero.
moverAux((Ficha, Fila, Columna), Numero, Tablero, Mov, Res):-	NuevoNumero is Numero-1, NuevaFila is Fila +1, NuevaFila =< 5,%moverse una fila
															...
															moverAux((Ficha, NuevaFila, Columna), NuevoNumero, Tablero, NuevoMov, Res).%recursivamente llamar hasta que se hayan movido todas las casillas necesarias
moverAux((Ficha, Fila, Columna), Numero, Tablero, Mov, Res):-	NuevoNumero is Numero-1, NuevaFila is Fila -1, NuevaFila >= -1,
															libre(Tablero, NuevaFila, Columna, NuevoNumero),
															valido(NuevaFila, Columna, Mov),
															append(Mov, [(NuevaFila, Columna)], NuevoMov),
															moverAux((Ficha, NuevaFila, Columna), NuevoNumero, Tablero, NuevoMov, Res).
moverAux((Ficha, Fila, Columna), Numero, Tablero, Mov, Res):-	NuevoNumero is Numero-1, NuevaColumna is Columna+1, NuevaColumna =< 5,
															libre(Tablero, Fila, NuevaColumna, NuevoNumero),
															valido(Fila, NuevaColumna, Mov),
															append(Mov, [(Fila, NuevaColumna)], NuevoMov),
															moverAux((Ficha, Fila, NuevaColumna), NuevoNumero, Tablero, NuevoMov, Res).
moverAux((Ficha, Fila, Columna), Numero, Tablero, Mov, Res):-	NuevoNumero is Numero-1, NuevaColumna is Columna-1, NuevaColumna >= 0,
															libre(Tablero, Fila, NuevaColumna, NuevoNumero),
															valido(Fila, NuevaColumna), Mov),
															append(Mov, [(Fila, NuevaColumna)], NuevoMov),
															moverAux((Ficha, Fila, NuevaColumna), NuevoNumero, Tablero, NuevoMov, Res).
...
movimientoNormal([(NumFicha,Fila,Columna) | RestoFichas], Tablero, Turno, TurnoOponente, NuevoTablero, Solucion) :- 
										%guitracer,trace,	
										filaMover(Turno, Tablero, NumFila), 
										Fila=NumFila, mover((NumFicha,Fila,Columna), Tablero, NuevoTablero), last(NuevoTablero,NuevaFicha), 
...
ganador(Ficha, Tablero, Turno, NuevoTablero) :- Turno = 1, mover(Ficha, Tablero, NuevoTablero), last(NuevoTablero,Sol), Sol=(_,Fila,_), Fila = -1.
ganador(Ficha, Tablero, Turno, NuevoTablero) :- Turno = 2, mover(Ficha, Tablero, NuevoTablero), last(NuevoTablero,Sol), Sol=(_,Fila,_), Fila = 5.
